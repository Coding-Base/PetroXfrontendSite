...
export default function CourseWaitingPage() {
  ...
  // Force re-render when time is up to ensure button shows on mobile
  useEffect(() => {
    if (isTimeUp) {
  ...
  // Handle visibility change to ensure timer updates when tab comes back into focus
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        // Force re-render when tab comes back into focus
        setRenderKey(prev => prev + 1);
      }
    };
  ...
  return (
    <div 
      className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 p-4 flex items-center justify-center" 
      key={renderKey}
    >
      {/* Updated card container:
        Added max-h-[95vh] and overflow-y-auto to create a scrollable container on mobile.
        Adjusted padding for mobile (p-6) vs desktop (md:p-12).
      */}
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl p-6 md:p-12 flex flex-col max-h-[95vh] overflow-y-auto overscroll-contain">
        ...
        {/* Course Info */}
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8 flex-shrink-0">
        ...
        {!isTimeUp ? (
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-8 text-center flex-shrink-0">
        ...
        {/* Exam Details */}
        <div className="grid grid-cols-2 gap-4 mb-8 bg-gray-50 rounded-lg p-6 flex-shrink-0">
          <div>
        ...
        {/* Instructions */}
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 mb-8 flex-shrink-0">
        ...
        {/* Action Buttons */}
        <div className="mt-auto space-y-3 flex-shrink-0 pt-4">
          <button
          ...
          <button
            onClick={() => navigate('/dashboard/enrolled-courses')}
            className="w-full py-2 px-6 rounded-lg font-medium text-gray-600 border border-gray-300 hover:bg-gray-50 transition-all"
          >
            Back to Courses
          </button>
        </div>
