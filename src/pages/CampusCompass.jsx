...
const CampusCompass = () => {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    ...
                    setDirectionsError('Could not get your location. Using campus center as default.');
                },
                { enableHighAccuracy: true, timeout: 10000, maximumAge: 10000 }
            );
        } else {
            setUserPosition(getCenter());
            setDirectionsError('Geolocation not supported. Using campus center as default.');
        }
    }, [getCenter]);
    ...
    useEffect(() => {
        if (isMobile && selectedLocation) {
            setMapPadding({ top: 0, right: 0, bottom: 200, left: 0 });
        } else {
            setMapPadding({ top: 0, right: 0, bottom: 0, left: 0 });
        }
    ...
    const handleGetDirections = useCallback(() => {
        if (selectedLocation) {
            calculateDirections(selectedLocation);
        }
    }, [selectedLocation, calculateDirections]);
