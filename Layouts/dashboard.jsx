    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (history.data.length) {
          const tot = history.data.reduce((acc, s) => acc + s.score, 0);
        ...
        // Rank
        const r = await fetchUserRank();
        setUserRank(r.data.rank);
        setIsLoading(s => ({ ...s, rank: false }));

        // Upload stats
        const us = await fetchUserUploadStats();
        const approved = us.data.approved_uploads || 0;
              ...
              <div className="bg-white p-4 md:p-6 rounded-xl shadow-md">
                ...
                <div className="h-64 md:h-80 flex items-center justify-center relative">
                  {isLoading.history ? (
                    ...
                    <div className="text-gray-500 text-center">No test data available</div>
                  )}
                </div>
              </div>
                  ...
                  {isLoading.leaderboard ? (
                    ...
                    leaderboard.slice(0, 5).map((u, idx) => (
                      <div key={u.id || idx} className="flex items-center p-3 mb-2 bg-white rounded-lg shadow-sm">
                        ...
                        <div className="font-semibold text-blue-600">{Math.round(u.avg_score)}%</div>
                      </div>
                    ))
                  ) : (
